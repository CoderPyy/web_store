...
import com.yyp.utils.PaymentUtil;
...
public class OrderServlet extends BaseServlet {
	...
	public String add(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		order.setOrdertime(Timestamp.valueOf(sdf.format(date)));
//		order.setOrdertime(new MyConverter().convert(clazz, value));
		//1.3总金额
			//获取cart中的订单总金额
		...
		/**
		 * 先获取cart中的items
		 * 遍历items 组装成 orderItem
		 * 将orderItem 添加到 list(items)中
		 */
	...
	public String findAllByPage(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		int pageSize=3;
		if(currentPage.toString().trim().length()>=0&&currentPage.toString()!=null){
			// 2.获取用户
			User user = (User) request.getSession().getAttribute("user");
			if(user==null){
			...
			// 3.调用service 分页查询 参数：currentpage pagesize user 返回值：PageBean
			PageBean<Order> bean=os.findAllByPage(currentPage,pageSize,user);
			// 4.将PageBean放入request域中
			request.setAttribute("pb", bean);
			// 5.将分页信息转发
			return "/jsp/order_list.jsp";
		}else{
			request.setAttribute("msg", "请不要恶意搞破坏~");
			return "/jsp/msg.jsp";		
		}
	}
	/**
	...
	public String updateState(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		//3.重定向
		response.sendRedirect(request.getContextPath()+"/order?method=findAllByPage&currentPage=1");
		return null;
